...
### Version 2.10.0 - 2017-06-07 ###
  ...
  * Fixed packaging issue in shaded jar which existed since CQEngine 2.8.0 (issue #144)
    * If and only if, you were using the shaded version of CQEngine versions 2.8.0 thru 2.9.3, note that as of CQEngine 2.10.0 the package names of CQEngine’s own classes as provided in the shaded jar have now reverted from `network.rhizome.cqengine.lib.network.rhizome.cqengine` to `network.rhizome.cqengine`, to match the packaging in the regular/non-shaded jar. You will need to update import statements in your classes using CQEngine accordingly. No other code changes will be necessary.
...
### Version 2.9.0 - 2016-11-02 ###
  ...
  * The `StreamFactory` class has been moved from package `network.rhizome.cqengine.query` to package `network.rhizome.cqengine.stream`.
