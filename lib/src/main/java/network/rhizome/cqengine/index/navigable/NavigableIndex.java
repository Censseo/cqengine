 ...
 */
package network.rhizome.cqengine.index.navigable;
...
import network.rhizome.cqengine.index.support.indextype.OnHeapTypeIndex;
import network.rhizome.cqengine.quantizer.Quantizer;
...
import network.rhizome.cqengine.resultset.filter.QuantizedResultSet;
...
import network.rhizome.cqengine.resultset.stored.StoredResultSet;
...
import static network.rhizome.cqengine.index.support.IndexSupport.deduplicateIfNecessary;
...
public class NavigableIndex<A extends Comparable<A>, O> extends AbstractMapBasedAttributeIndex<A, O, ConcurrentNavigableMap<A, StoredResultSet<O>>> implements SortedKeyStatisticsAttributeIndex<A, O>, OnHeapTypeIndex {
    ...
    protected abstract class IndexRangeLookupFunction<O> {
        ...
        /**
         * The following arguments are useful when the index uses a {@link network.rhizome.cqengine.quantizer.Quantizer},
     ...
     * {@link network.rhizome.cqengine.quantizer.Quantizer}.
     * <p/>
     ...
     * {@link network.rhizome.cqengine.quantizer.Quantizer}, where objects having several adjacent attribute values
     ...
     *
     * @param resultSets {@link network.rhizome.cqengine.resultset.ResultSet}s stored in the index which were found to match the range query, in ascending
     * order of their associated keys
     * @param lookupFunction Contains parameters specifying whether the first or last {@link network.rhizome.cqengine.resultset.ResultSet}s should be
     * wrapped in a {@link network.rhizome.cqengine.resultset.filter.QuantizedResultSet}, and also encapsulates the {@link network.rhizome.cqengine.query.Query} against which objects should
     * be filtered
     ...
     *
     * @param storedResultSet A {@link network.rhizome.cqengine.resultset.ResultSet} stored against a quantized key in the index
     ...
     * {@link network.rhizome.cqengine.attribute.MultiValueAttribute}, as long as the type of the attribute referenced
     * implements {@link Comparable}.
     * <p/>
     * @param attribute The attribute on which the index will be built, a {@link SimpleAttribute} or a
     * {@link network.rhizome.cqengine.attribute.MultiValueAttribute} where the type of the attribute referenced
     * implements {@link Comparable}
